...
| `selectedItemIndex` | `number?` | Zero-based row of a rendered [`list` element](/app-devs/core-concepts/display/layouts#lists) the gesture landed on. Absent for plain touchpad gestures. |
| `selectedItemName` | `string?` | That row's text, present together with `selectedItemIndex`. |
...
On the Even Realities G2, a double tap normally opens the glasses' own
dashboard. While a running miniapp listens for touches (any `onTouch`, or
`onTouch("double_tap", ...)`), the gesture goes to the miniapp instead and the
