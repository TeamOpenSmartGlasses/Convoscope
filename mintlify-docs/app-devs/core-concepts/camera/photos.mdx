...
| `compress` | `"none" \| "low" \| "medium" \| "high"` | `"none"` | JPEG quality tier applied before upload (`none` Q95 over BLE / untouched over Wi-Fi, `low` Q88, `medium` Q78, `high` Q60). Does not change dimensions; use `size` for that. |
