...
| `compress` | React Native, Android, iOS | JPEG compression tier: `none` (Q95 over BLE; Wi-Fi uploads the capture untouched), `low` (Q88), `medium` (Q78), or `high` (Q60). `heavy` is a legacy alias for `high`. Compression changes encoder quality only; `size` alone controls pixel dimensions. Omitted values default to `none` on every platform. |
...
// Reconcile state after app resume, a timeout, or a reconnect.
const current = await BluetoothSdk.queryVideoRecordingStatus(`status-${Date.now()}`);
console.log('recording now', current.data?.recording, current.data?.duration_ms);
...
val current = sdk.queryVideoRecordingStatus("status-${System.currentTimeMillis()}")
println("recording now: ${current.data?.get("recording")}")
...
let current = try await sdk.queryVideoRecordingStatus(requestId: "status-\(Date().timeIntervalSince1970)")
print("recording now: \(current.data?["recording"] ?? false)")
...
`startVideoRecording(...)` resolves from `recording_started`. `stopVideoRecording(...)` without a webhook resolves from `recording_stopped`; when you pass a webhook URL, it waits for `recording_stopped` plus video `media_success`, and rejects on video `media_error`. `queryVideoRecordingStatus(requestId)` sends the glasses' existing status query and resolves with the current `recording` state and elapsed `duration_ms`, so use it to reconcile after a timeout, reconnect, or app resume. Use a fresh request ID for each query. Raw `video_recording_status`, `media_success`, and `media_error` events remain available for listeners that need the glasses event stream directly. The video upload webhook should accept multipart form data with a `video` field and `requestId`. Pass the webhook URL and auth token at stop time so upload credentials can be fresh when the recording ends.
...
| `compress` | Compression tier for action-button photos: `none`, `low`, `medium`, or `high` (`heavy` is a legacy alias for `high`; the glasses store and acknowledge the canonical tier). |
