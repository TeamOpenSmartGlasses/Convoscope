...
const CONFIGURE_SCRIPT_SOURCE = fileURLToPath(new URL("./configure-bluetooth-sdk-ota-manifest.mjs", import.meta.url))
...
function collectArtifactReferences(manifest) {
  ...
  if (manifest.mtk_full_ota) {
    ...
    if (
      typeof entry.end_firmware !== "string" ||
    ...
    references.push({
      ...
      key: "url",
      label: "MTK full OTA",
      sha256: requireSha256(entry, "MTK full OTA"),
      size: entry.size,
    })
  }
...
async function main() {
  ...
  const localArtifacts = process.env.LOCAL_ARTIFACTS_JSON ? JSON.parse(process.env.LOCAL_ARTIFACTS_JSON) : {}
